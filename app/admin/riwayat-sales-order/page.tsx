...
//   // 🔹 Fungsi untuk menghapus sales order
...
//       const res = await api.delete(`/private/sa/sales-order`, {
//         data: { id: orderId },
//       });

//       if (res.data.status === 200) {
//         showSuccessToast("Sales order berhasil dihapus");
//         // Refresh data setelah hapus
//         fetchSalesOrders(search, sortOrder === "desc");
//       }
//     } catch (err) {
//       console.error("Gagal menghapus sales order:", err);
//     }
//   };
...
//                 <th className="text-left py-3 px-3 w-[15%]">SO Number</th>
...
//                 <th className="text-center py-3 px-3 w-[5%]">Aksi</th>
//               </tr>
//             </thead>
//             <tbody>
//               {loading ? (
...
//                         <Button variant="destructive" size="sm" className="p-2" title="Hapus" onClick={(e) => handleDeleteOrder(order.id, e)}>
//                           <Image src="/assets/delete.png" alt="Hapus" width={20} height={20} />
//                         </Button>
//                       </div>
//                     </td>
//                   </tr>
...
import { api } from "@/lib/axios";
import Cookies from "js-cookie";
import { ArrowUpDown, Copy, Search, SlidersHorizontal, Trash2 } from "lucide-react";
import Image from "next/image";
import { useEffect, useState } from "react";
import { VisuallyHidden } from "@radix-ui/react-visually-hidden";
...
export default function SalesOrderPage() {
  ...
  const handleDeleteClick = (order: SalesOrder, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Gunakan endpoint DELETE dengan ID sales order
      const res = await api.delete(`/private/sa/sales-order`, {
        data: {
          id: orderToDelete.id,
        }
      ...
      if (res.data.status === 200) {
        ...
        // Tunggu sebentar untuk menampilkan success message sebelum menutup dialog
        setTimeout(() => {
        ...
        throw new Error(res.data.message || "Gagal menghapus sales order");
      }
    } catch (err: any) {
      console.error("Gagal menghapus sales order:", err);
      const errorMessage = err.response?.data?.message || "Terjadi kesalahan saat menghapus sales order";
              ...
              {loading ? (
                ...
                salesOrders.map((order, i) => (
                  <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50 transition cursor-pointer" onClick={() => handleRowClick(order)}>
                    ...
                    <td className="py-3 px-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex justify-center gap-1">
                        <Button variant="destructive" size="sm" className="p-2" title="Hapus" onClick={(e) => handleDeleteClick(order, e)}>
          ...
          {deleteStep === "confirm" ? (
            <>
              <AlertDialogHeader>
                ...
                <AlertDialogTitle className="text-center">Ingin menghapus sales order?</AlertDialogTitle>
              </AlertDialogHeader>
              <AlertDialogFooter className="bg-gray-100 -mx-6 p-2 -mb-6 rounded-b-md justify-center space-x-2">
                <Button variant="destructive" onClick={handleCloseDeleteDialog}>
                  Batal
                </Button>
                <Button className="bg-[#0892D8] hover:bg-[#0892D8]/90" onClick={handleConfirmDelete} disabled={deleting}>
            ...
            <>
              <AlertDialogHeader>
                ...
                <AlertDialogTitle className="text-center">Berhasil menghapus sales order</AlertDialogTitle>
              </AlertDialogHeader>
            </>
          )}
        </AlertDialogContent>
