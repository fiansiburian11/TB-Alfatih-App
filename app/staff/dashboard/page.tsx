...
export default function DashboardStaff() {
  ...
  const fetchInti = async (page = currentPageInti, limit = itemsPerPageInti, filterParams = filters) => {
    try {
      ...
      const res = await api.get("/private/product?desc=true&active=true", { params });
  ...
  //effect untuk 3 card diatas dashboard
  useEffect(() => {
    nameUser();
    fetchTotalSO();
  }, []);
  ...
  const handleRowClick = (productId: string) => {
    router.push(`/staff/katalog-produk/${productId}`);
  };
  ...
  return (
    <div className="mb-8 space-y-5">
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full">
            ...
            <div className="flex flex-col justify-between bg-white p-4 rounded-md shadow-md flex-1">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-xs text-gray-600">Telusuri seluruh produk di katalog untuk ditawarkan ke pelanggan</p>
              </div>

              <div className="flex flex-col gap-2 mt-4">
                <Button
                  onClick={() => {
                    router.push("/staff/katalog-produk");
                  }}
            ...
            <div className="flex flex-col justify-between bg-white p-4 rounded-md shadow-md flex-1">
              ...
              <div className="flex flex-col gap-2 mt-4">
                <Button
                  onClick={() => {
                    router.push("/staff/riwayat-sales-order");
                  }}
                  className="flex justify-between bg-accent hover:bg-slate-200 text-black"
                >
                  Lihat Riwayat SO
      ...
      <div className="bg-white rounded-md p-2 space-y-2 shadow-sm">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {barangInti.map((item, index) => {
                    ...
                    return (
                      <tr key={item.id} className={`cursor-pointer hover:bg-gray-50 transition-colors ${index === barangInti.length - 1 ? "" : "border-b"}`} onClick={() => handleRowClick(item.id)}>
                        ...
                        <td className="py-3 px-4 text-sm whitespace-pre-line">{item.kondisi_peruntukan || "-"}</td>
                        <td className="py-3 px-4 text-sm">{fmtRp.format(item.harga_jual || 0)}</td>
                        <td
                          className="py-3 px-4 text-center"
                          onClick={(e) => e.stopPropagation()} // Mencegah navigasi saat klik draft
                        >
              ...
              <table className="w-full">
                ...
                <tbody>
                  {crossProducts.map((item, index) => {
                    ...
                    return (
                      <tr key={item.id} className={`cursor-pointer hover:bg-gray-50 transition-colors ${index === crossProducts.length - 1 ? "" : "border-b"}`} onClick={() => handleRowClick(item.id)}>
                        <td className="py-3 px-4 font-medium">
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                              <span>{item.name}</span>
